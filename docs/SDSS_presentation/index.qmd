...
bibliography: "SDSS2023_Abstract.bib"
---
...
In 1990, Microsoft Excel 3 introduced the ability to create 3D charts [@walkenbach_versions_2020]. Multiple studies have shown the downsides to unnecessarily using these types of charts.

  - 2D graphs had more confident answers and novice participants were more accurate with 2D graphs than 3D graphs [@barfield_effects_1989]
  
  - Visual appeal for 2D and 3D graphs were approximately equal, and simple graphs were preferred for extracting information [@fisher_data_1997]
...
Elementary perceptual tasks are various elements from graphs that people use to extract information from graphs, such as length, angle, position, and area.

:::

## Replicating Cleveland and McGill

Our study draws influence from Cleveland and McGill's 1984 paper, specifically from their position-length experiment with graph types 1 and 3 [@cleveland_graphical_1984].
...
ggplot(mapping = aes(x = 1:10, y = val)) +
  geom_bar(stat = 'identity', width = 1,
           color = 'black',
           fill = 'white') +
  theme_bw() +
  labs(title = 'Bars for comparison',
       y = expression(V[i]),
       x = 'i') +
  scale_x_continuous(breaks = 1:10) +
  scale_y_continuous(breaks = round(val,1), limits = c(0, 60)) +
...
![Results averaged over ratio, holding the comparison type constant](images/lsmeans-glmm.png){fig-align='center'}
